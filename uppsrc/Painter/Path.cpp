...
void PainterFrontend::ClearPath()
{
	path.Clear();
	current = move = Null;
...
Pointf PainterFrontend::PathPoint(const Pointf& p, bool rel)
{
	Pointf r;
...
Pointf PainterFrontend::EndPoint(const Pointf& p, bool rel)
{
...
PainterFrontend::PathElement& PainterFrontend::PathAddRaw(int type, int size)
{
	...
	path.SetLength(l + size);
	PathElement *e = (PathElement *)(~path + l);
	e->type = type;
	return *e;
}

void PainterFrontend::MoveOp(const Pointf& p, bool rel)
{
	LLOG("@ MoveOp " << p << ", " << rel);
	PathAdd<LinearData>(MOVE).p = move = ccontrol = qcontrol = EndPoint(p, rel);
}

void PainterFrontend::DoMove0()
{
	if(IsNull(move))
		MoveOp(Pointf(0, 0), false);
}

void PainterFrontend::LineOp(const Pointf& p, bool rel)
{
	LLOG("@ LineOp " << p << ", " << rel);
	DoMove0();
...
void PainterFrontend::QuadraticOp(const Pointf& p1, const Pointf& p, bool rel)
{
...
void PainterFrontend::QuadraticOp(const Pointf& p, bool rel)
{
	QuadraticOp(2.0 * current - qcontrol, p, rel);
}

void PainterFrontend::CubicOp(const Pointf& p1, const Pointf& p2, const Pointf& p, bool rel)
{
...
void PainterFrontend::CubicOp(const Pointf& p2, const Pointf& p, bool rel)
{
	CubicOp(2.0 * current - ccontrol, p2, p, rel);
}

void PainterFrontend::ArcOp(const Pointf& c, const Pointf& r, double angle, double sweep, bool rel)
{
...
void PainterFrontend::SvgArcOp(const Pointf& r, double xangle, bool large, bool sweep,
...
void PainterFrontend::DivOp()
{
	LLOG("@ DivOp");
	CloseOp();
	PathAdd<PathElement>(DIV);
}

void PainterFrontend::CharacterOp(const Pointf& p, int ch, Font fnt)
{
